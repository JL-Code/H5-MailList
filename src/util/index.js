...
objectAssign($.fn, {
  ...
  on: function(eventType, selector, handler) {
    const isDelegate =
      typeof selector === "string" && typeof handler === "function";
    if (!isDelegate) {
      handler = selector;
    }
    console.debug("on:this", this);
    this.forEach($element => {
      console.debug("on:this foreach element", $element);
      eventType.split(" ").forEach(event => {
        $element.addEventListener(event, function(evt) {
          console.debug("addEventListener this", this);
          if (isDelegate) {
